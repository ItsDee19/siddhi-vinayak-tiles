...
// Confirmed by the showroom owner; preview deployment hosts are never canonical.
...
// An explicit owner-supplied URL wins; otherwise keep the confirmed public
// domain even when Vercel exposes a default project or per-deployment hostname.
export function resolveSiteOrigin(env = {}) {
  const configured = env.SITE_URL?.trim()
  const value = configured || PRODUCTION_ORIGIN
  let url
...
export function structuredData({ origin = PRODUCTION_ORIGIN, path = '/', title = siteTitle, description = siteDescription } = {}) {
  ...
  return {
    ...
    '@graph': [
      ...
      {
        '@type': path.startsWith('/catalogue/') ? 'CollectionPage' : 'WebPage', '@id': `${pageUrl}#webpage`,
...
export function renderSeoHead({
  ...
  preview = false, noindex = false, canonical = true, includeStructuredData = true,
} = {}) {
  const tags = [
  ...
  if (includeStructuredData) tags.push(`<script type="application/ld+json">${serializeJsonLd(structuredData({ origin, path, title, description }))}</script>`)
  return tags.join('\n    ')
}
...
export function renderStaticHeader() {
  return `<header class="container-px border-b border-white/10 py-5"><nav aria-label="Main navigation" class="flex flex-wrap items-center justify-between gap-4">
    ...
    <div class="flex flex-wrap gap-x-5 gap-y-2">
      <a href="/catalogue/" class="inline-flex min-h-11 items-center text-sm text-gold hover:underline">Catalogue</a>
      <a href="/#contact" class="inline-flex min-h-11 items-center text-sm text-gold hover:underline">Contact the showroom</a>
    </div>
  </nav></header>`
}

export function renderStaticFooter() {
  return `<footer class="container-px border-t border-white/10 py-8 text-sm text-sand">
    ...
    <div class="mt-3 flex flex-wrap gap-x-5 gap-y-2">
      <a href="tel:${escapeHtml(business.phoneTel)}" class="inline-flex min-h-11 items-center text-gold hover:underline">Call ${escapeHtml(business.phoneDisplay)}</a>
      <a href="/#visualizer" class="inline-flex min-h-11 items-center text-gold hover:underline">Try the 3D room visualizer</a>
    </div>
    <nav aria-label="Website information" class="mt-3 flex flex-wrap gap-x-5 gap-y-2">
      ${policyLinks.map(link => `<a href="${link.href}" class="inline-flex min-h-11 items-center text-sand underline underline-offset-4 hover:text-cream">${link.label}</a>`).join('')}
    </nav>
  </footer>`
}
...
function productArticle(product) {
  ...
  const imageAttributes = Object.entries(imageProps)
    .map(([name, value]) => `${name === 'srcSet' ? 'srcset' : name}="${escapeHtml(value)}"`).join(' ')
  const image = typeof product.imageUrl === 'string' && /^\/(?!\/)/.test(product.imageUrl)
    && !product.imageUrl.includes('\\')
    ? `<img ${imageAttributes} alt="${escapeHtml(product.name)}" loading="lazy" decoding="async" class="aspect-[4/3] w-full object-contain" />`
  ...
  return `<article class="overflow-hidden rounded-card border border-white/10 bg-charcoal-700">
    ...
    <div class="p-4"><p class="text-xs text-sand">${escapeHtml(product.category)}</p>
      ...
      <a href="${href}" class="mt-3 inline-flex min-h-11 items-center text-sm font-semibold text-gold-light hover:underline">View details</a>
    </div></article>`
}
...
export function renderCatalogueBody(page) {
  ...
  return `<a href="#catalogue-products" class="skip-link">Skip to products</a>
    ...
    <main id="catalogue-products" class="container-px py-12" tabindex="-1">
      ...
      <a href="/#catalogue" class="mt-2 inline-flex min-h-11 items-center text-sm text-gold underline underline-offset-4">Search &amp; filter catalogue</a>
